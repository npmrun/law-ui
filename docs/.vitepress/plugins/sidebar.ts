...
export default class VitePressSidebar {
  static generateSidebar(options?: Options | Options[]): Sidebar {
    ...
    for (let i = 0; i < optionItems.length; i += 1) {
      ...
      const readItems = function (items, base) {
        items.forEach(item=>{
            if(item.link && optionItem.flattenBase) { // 抹除base
                item.link = base + item.link
            }
            if(item.items && item.items.length) {
                readItems(item.items, base)
            } else if(item.items && item.link && optionItem.removeEmptyFolder){
                delete item.collapsed
                delete item.items
            }
        })
      }
      readItems(sidebar[optionItem.resolvePath || '/'].items, sidebar[optionItem.resolvePath || '/'].base)
      optionItem.flattenBase && (sidebar[optionItem.resolvePath || '/'] = sidebar[optionItem.resolvePath || '/'].items as any)
      sidebar[optionItem.resolvePath || '/'] = optionItem.callback?.(sidebar[optionItem.resolvePath || '/']) ?? sidebar[optionItem.resolvePath || '/']
    }
      ...
      .map((x: string) => {
        ...
        if (statSync(childItemPath).isDirectory()) {
          ...
          if (options.convertSameNameSubFileToGroupIndexPage) {
            const findItem = directorySidebarItems.find((y: SidebarListItem) => (y.text === x && y.items === undefined));
        ...
        if (childItemPath.endsWith('.md')) {
          ...
          childItemPathDisplay = childItemPathDisplay.replace(/index$/,"")
          return {
            text: childItemText,
  ...
  private static formatTitle(options: Options, title: string): string {
    ...
    if(options.dirTitleMap && options.dirTitleMap[title]){
      result = options.dirTitleMap[title]
    }
